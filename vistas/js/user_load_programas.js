...
const cargaMisSecciones = (()=> {
    
    for(let icat=0; icat<miLista.length; icat++){
    ...
    sectionNuestrosProg.classList.add('nuestros-programas');
    {
        ...
        divTitulo.classList.add('contenedor-titulo-categoria');
        {
            ...
            h3.innerText = miLista[icat].categoria;
        ...
        for(let i=0; i<miLista[icat].programas.length; i++){
            {
                let articulo = document.createElement('article');
                console.log('es', miLista[icat].tono);
                articulo.classList.add('contenedor-programa-m', miLista[icat].tono);
                {
                    let divProgPortada = document.createElement('div');
                    divProgPortada.classList.add('programa-m-portada');
                    {
                        let img = document.createElement('img');
                        img.src = miLista[icat].programas[i].imagenSrc;
                    ...
                    divProgTitulo.classList.add('programa-m-titulo');
                    {
                        let p = document.createElement('p');
                        {
                            ...
                            stroH.innerText = miLista[icat].programas[i].nombre;
                            p.appendChild(stroH);
                        }
                    ...
                    divProgHorario.classList.add('programa-m-horarios');
                    {
                        ...
                        pHor.innerText = miLista[icat].programas[i].horario;
                    ...
                    divProgMasDetalles.classList.add('programa-m-mas-detalles');
                    {
                        ...
                        <span>${miLista[icat].programas[i].ultimoVideo}</span>`;
                        }
                        ...
                        let pIdiomas = document.createElement('p');
                        {
                            let svgAll =`<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"viewBox="0 0 510 510" style="enable-background:new 0 0 510 510;" xml:space="preserve">
                                        ...
                                        <span>${miLista[icat].programas[i].idiomas[0].idioma}</span>`;
                            //spanUltumaEm.innerText = miLista[icat].programas[i].idiomas[0].idioma;
                        ...
                        <span>${miLista[icat].programas[i].tiempo}</span>`;
                        }
    ...
    .then((data) => {
        if(data.code=='1'){
            ...
            if(data.data.length>0){
                ...
                let anteripr = -1;
                let progTmp = new Array();
                miLista = new Array();
                progTmp.push({
                    nombre: data.data[0].nombre_programa,
                    descripcion: data.data[0].descripcion,
                    horario: `${data.data[0].dias_de_emision}, ${data.data[0].hora_inicio_emision}-${data.data[0].hora_inicio_emision}`,
                    idiomas: [{idioma: data.data[0].idioma}],
                    tiempo: data.data[0].duracion_promedio,
                    ultimoVIdeo: "none",
                    imagenSrc: `${URL.substring(0, URL.length-4)}/storage/${data.data[0].ruta_imagen}` 
                });
                let categoriaIndexAxtual = 0;
                for (let index = 1; index < data.data.length; index++) {
                    const element = data.data[index];
                    if(data.data.id_categoria!=anteripr){
                        miLista.push(
                            {
                                categoria: data.data[categoriaIndexAxtual].nombre_categoria,
                                tono: data.data[categoriaIndexAxtual].nombre_clase_color,
                                programas: progTmp
                            }
                        );
                        progTmp = new Array();
                        categoriaIndexAxtual = index;
                        anteripr = data.data.id_categoria;
                    } 
                    
                    progTmp.push({
                        nombre: data.data[index].nombre_programa,
                        descripcion: data.data[index].descripcion,
                        horario: `${data.data[index].dias_de_emision}, ${data.data[index].hora_inicio_emision}-${data.data[index].hora_inicio_emision}`,
                        idiomas: [{idioma: data.data[index].idioma}],
                        tiempo: data.data[index].duracion_promedio,
                        ultimoVIdeo: "none",
                        imagenSrc: `${URL.substring(0, URL.length-4)}/storage/${data.data[index].ruta_imagen}` 
                    });
                }
                miLista.push({
                    categoria: data.data[categoriaIndexAxtual].nombre_categoria,
